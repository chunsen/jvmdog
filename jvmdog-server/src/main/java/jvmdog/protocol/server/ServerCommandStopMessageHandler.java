...
@Service
public class ServerCommandStopMessageHandler implements ServerMessgeHandler{
    private static final Logger logger = LoggerFactory.getLogger(ServerCommandStopMessageHandler.class);
    ...
    public void handle(DogMessage inputMessage, DogConnection connection) {
        try {
            ...
            logger.info("ServerCommandStop: {} from {}", cmd, connection.id());
    ...
    @Override
    public int type() {
        return DogMessageType.CLIENT_COMMAND_STOP_RESPONSE.getValue();
    }
